...
**Practice exams** (`content/practice_exams/` + `backend/content_pipeline/practice.py` + `frontend/src/practice/`): a separate, AI-written (Opus 5.5) track of 15 practice exams for AZ-104, AZ-700 and AZ-305, compiled to `frontend/public/content/practice_exams/`. They carry no reputation, save state or objective coverage; attempts persist under their own localStorage key. Every question must cite learn.microsoft.com, and the "written by Opus 5.5" labeling on the pages, README, About and terms must stay.
...
- **Diagram ids are save-state keys.** Player saves store living-map state keyed by node/edge id, so ids in `content/diagrams/act{1,2}.yaml` are permanent once shipped. Ids are deliberately decoupled from labels (`id: st-pod` / `label: stveymarkpod`). Rename the label, never the id — or add the old-to-new pair to `RENAMED_DIAGRAM_IDS` in `migrateSave` (`engine.ts`) if an id genuinely must change, as was done when the company was renamed. The same applies to quest and encounter ids, which key progress.
